  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        action={<Link href="/app/properties/new"><Button size="md"><IconPlus className="w-4 h-4" /> Add Property</Button></Link>}
      />

      {allProperties.length === 0 ? (
        <EmptyState icon={<IconBuilding />} title="No properties yet"
        ...
        <div className="space-y-3">
          {allProperties.map((property, i) => {
            ...
            const overdue = propertyCompliance.filter(c => c.status === 'overdue').length;

            return (
              <Link key={property.id} href={`/app/properties/${property.id}`}>
                <div className="relative overflow-hidden flex items-center gap-4 p-5 rounded-2xl bg-white border border-slate-200/60 transition-all duration-500 hover:translate-y-[-4px] hover:shadow-[0_20px_50px_rgba(99,102,241,0.12),0_0_0_1px_rgba(99,102,241,0.08)] animate-slide-up"
                  style={{ animationDelay: `${i * 75}ms` }}>
                  <div className="absolute top-0 right-0 w-24 h-24 bg-brand-50/30 rounded-full -translate-y-1/2 translate-x-1/3" />
                  ...
                  <div className="relative flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="text-sm font-bold text-slate-900 truncate">{property.nickname}</h3>
                        <p className="text-xs text-slate-500 truncate mt-0.5">{property.address_line_1}, {property.postcode}</p>
                      </div>
                    ...
                    <div className="flex items-center flex-wrap gap-2 mt-2.5">
                      ...
                      {activeTenant && (
                        <Badge variant="info"><span className="flex items-center gap-1"><IconUser className="w-3 h-3" />{activeTenant.tenant_name.split(' ')[0]}</span></Badge>
                      )}
