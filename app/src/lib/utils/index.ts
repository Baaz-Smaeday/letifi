...
// ============================================
export function calculatePropertyScore(records: ComplianceRecord[], enabledTypes?: string[] | null): number {
  // If landlord has chosen specific types, only score those
  const typesToScore = enabledTypes && enabledTypes.length > 0
    ? enabledTypes as ComplianceType[]
    : ALL_COMPLIANCE_TYPES;

  if (typesToScore.length === 0) return 100; // nothing to track = perfect

  let score = 0;
  const perItem = 100 / typesToScore.length;
...
export function calculateOverallScore(allRecords: ComplianceRecord[], propertyIds: string[], propertiesMap?: Map<string, string[] | null>): number {
  if (propertyIds.length === 0) return 0;
  let total = 0;
  for (const pid of propertyIds) {
    const enabledTypes = propertiesMap?.get(pid) || null;
    total += calculatePropertyScore(allRecords.filter((r) => r.property_id === pid), enabledTypes);
  }
